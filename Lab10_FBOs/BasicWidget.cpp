...
void BasicWidget::mouseMoveEvent(QMouseEvent* mouseEvent) {
  ...
  if (mouseAction_ == Rotate) {
    ...
    QVector3D c_position = camera_.position();

    c_position -= camera_.lookAt();
    float x = c_position.x();
    float z = c_position.z();
    float angle = qDegreesToRadians(0.5f * delta.x());
    c_position.setX((x * qCos(angle)) - (z * qSin(angle)));
    c_position.setZ((z * qCos(angle)) + (x * qSin(angle)));
    c_position += camera_.lookAt();

    camera_.setPosition(c_position);
    ...
    camera_.translateCamera(camera_.gazeVector() * 0.03f * -delta.y());
  }
  update();
}
...
// This is just a Unit Quad that will not ever get lit or have anything to do
// with normals!
...
void BasicWidget::setupShaders() {
  // TODO:  You may need to change these paths based on how/where you choose to
  // build
...
void BasicWidget::paintGL() {
  ...
  // TODO:  This is wasteful -- do we really NEED to create a new FBO every
  // frame?!
  QOpenGLFramebufferObject fbo(size(),
                               QOpenGLFramebufferObject::CombinedDepthStencil);

  // Bind our FBO.
  // This make our current render target the framebuffer that we have bound.  We
  // will not be seeing any imagery from now on!
  fbo.bind();
  ...
  // From now on, we are drawing to the "normal" framebuffer.  We will see in
  // our window whatever gets drawn here.
  makeCurrent();

  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

  // At this point, our FBO has our rendered scene in it.
  // We now want to do the second render pass to paste it onto a quad as if it
  // were a normal texture.
  // TODO -- Note, Qt doesn't expose the textures an QOpenGLFrameBufferObject
  // stores directly instead, it provides a method to get the textureID that it
  // used to render to. We now want to bind it and use it to render our
  // screen-sized quad
  ...
  QMatrix4x4 id;
  id.setToIdentity();
  // Except our projection.  Our projection we set up to be the same size (and
  // coordinates) as the quad we built
  QMatrix4x4 proj;
  ...
  // We can also save out the contents of our framebuffer object without ever
  // actually rendering it to the screen!  Qt provides some VERY easy ways of
  // doing this.  Please note that this is a pretty expensive process, though.
  // Memory transfers from GPU->CPU are extremely expensive.
