...
TerrainQuad::TerrainQuad()
    : lightPos_(0.5f, 0.5f, -2.0f),
      sign_(1500.0f),
      numIdxPerStrip_(0),
      numStrips_(0),
      heightTexture_(QOpenGLTexture::Target2D) {}
...
void TerrainQuad::init(const QString& textureFile) {
  ...
  for (unsigned int r = 0; r < numRows + 1; ++r) {
    for (unsigned int c = 0; c < numCols + 1; ++c) {
      ...
      float z = r * rowStep;
      float x = c * colStep;
      // TODO - Before changing anything in the shaders, we can get
      // heightmapping to work by changing this y coordinate.  Implement this
      // now to create a heightmap!
      float y = (float)heightImage
                    .pixelColor(z * (heightImage.height() - 1),
                                x * (heightImage.width() - 1))
                    .red();
      y /= sign_;
      // float y = 0.5;
  ...
  // All of our strips are coalesced into a single buffer.  This means we will
  // need to be careful about things in the drawing call!
  Renderable::init(pos, norm, texCoord, idx, textureFile);
  // We want to setup our height texture AFTER initialization of our primary
  // members/context
...
void TerrainQuad::draw(const QMatrix4x4& world, const QMatrix4x4& view,
  ...
  // TODO - After seeing the initial heightmap by querying in C++ the height
  // image uncommment these lines to change implementations to use the vertex
  // shader! We bind our height texture at Texture Unit 0
  ...
  // TODO - Uncomment these lines when youa re ready to move from C++
  // implementation to the GPU shader implementation.
  ...
  glDrawElements(GL_TRIANGLE_STRIP, numStrips_ * numIdxPerStrip_,
                 GL_UNSIGNED_INT, 0);
