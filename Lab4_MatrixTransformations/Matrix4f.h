...
class Matrix4f {
 public:
  ...
  // We make the halfHeight negative at [1][1] because 0 is the top of the
  // screen.
  ...
  void InitOrthographic(float left, float right, float bottom, float top,
  ...
  // Set index of matrix to a value
  void Set(unsigned int i, unsigned int j, float value) { m[i][j] = value; }

  // Retrieve value matrix.
  float Get(unsigned int i, unsigned int j) { return m[i][j]; }
